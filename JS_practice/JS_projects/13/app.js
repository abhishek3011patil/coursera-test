...
form.addEventListener("submit", e => {
  // because while we submit form its default behavior is to submit to the server this prevent that
  e.preventDefault();
  
  const value = parseInt(amount.value); // because form values are string
  console.log(value);
  const random = Math.floor(Math.random()* text.length);
  console.log(random);
  // isNaN check whether there is a input or not
  if(isNaN(value) || value <= 0 || value > 9)
  article.innerHTML = `<p class = result>${text[random]}</p>`;
  else{
    // slice function select the array from 0 to the value 
     const context = text.slice(0,value);
    let mappedarray = context.map((item)=>{
      return `<p class = result>${item}</p>`;

    });
    article.innerHTML = mappedarray.join("");
   }
